...
| 模块 | 功能描述 |
|------|----------|
| 基础信息编辑 | 模板名称、描述、版本、作者、标签 |
| 世界观构建器 | 可视化编辑世界设定，支持自定义字段 |
| 种族编辑器 | 定义种族属性、特性、外观描述模板 |
| 职业编辑器 | 定义职业技能树、初始装备、成长曲线 |
| 背景编辑器 | 定义背景故事、初始属性加成、特殊事件 |
| 规则配置器 | 配置战斗规则、技能规则、物品规则 |
| AI约束设置 | 设置AI行为边界、内容过滤、风格指导 |
| 场景设计器 | 设计初始场景、NPC布局、任务触发 |
| 主题定制器 | 自定义UI颜色、字体、背景样式 |
| 预览测试 | 实时预览模板效果，快速测试 |
