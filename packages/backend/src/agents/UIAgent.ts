...
const DYNAMIC_UI_SYSTEM_PROMPT = `你是动态UI生成专家，负责生成各种游戏界面的Markdown内容。

## 支持的UI类型
- welcome: 欢迎界面，展示角色创建结果和游戏开始信息
- notification: 系统通知，显示重要消息和提示
- dialog: 对话框，用于NPC对话和剧情交互
- enhancement: 装备强化界面，显示装备强化相关信息
- warehouse: 仓库/银行界面，管理物品存储
- shop: 商店界面，展示商品和交易
- custom: 自定义界面，根据需求灵活生成
...
:::
\`\`\`
type可选值: info, success, warning, error, achievement
...
## 输出要求
1. 只输出Markdown内容，不要包含其他解释
2. 使用上下文中的变量替换，格式为 {variable.path}
3. 确保所有action链接格式正确
4. 合理使用扩展组件增强交互体验`;
...
所有UI指令必须符合UIInstruction接口规范：
{
  ...
  async generateDynamicUI(params: {
    ...
    const id = `dynamic-ui-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

    const typePrompts: Record<DynamicUIType, string> = {
      welcome: '生成一个游戏欢迎界面，展示角色创建结果和游戏开始信息。',
      notification: '生成一个系统通知，显示重要消息和提示。',
      dialog: '生成一个对话框界面，用于NPC对话和剧情交互。',
      enhancement: '生成一个装备强化界面，显示装备强化相关信息。',
      warehouse: '生成一个仓库/银行界面，管理物品存储。',
      shop: '生成一个商店界面，展示商品和交易。',
      custom: '根据描述生成自定义界面。',
    };

    const userPrompt = this.buildDynamicUIPrompt(type, context, typePrompts[type]);
    ...
    this.addMemory(
      `Generated dynamic UI: type=${type}, id=${id}`,
      'assistant',
      5,
      { type, id, contextKeys: Object.keys(context) }
    );
  ...
  private buildDynamicUIPrompt(
    ...
    const typeSpecificInstructions: Record<DynamicUIType, string> = {
      welcome: `
请生成欢迎界面，包含以下内容：
- 使用 :::system-notify{type=welcome} 包裹整体内容
- 展示角色信息（种族、职业、背景）
- 显示初始状态（金币、物品数量、技能数量）
- 提供开始冒险的选项按钮
- 使用变量替换格式：{character.name}, {character.race} 等`,
      notification: `
请生成系统通知，包含以下内容：
- 使用 :::system-notify{type=info|success|warning|error} 包裹
- 简洁明了的消息标题和内容
- 可选的操作按钮`,
      dialog: `
请生成对话界面，包含以下内容：
...
请根据上述上下文数据生成${type}类型的Markdown内容。只输出Markdown，不要包含其他解释。`;
  }

  /**
   * 处理动态UI生成请求
   */
  private async handleGenerateDynamicUI(
    data: Record<string, unknown>
  ): Promise<AgentResponse> {
    try {
      const uiType = data.uiType as DynamicUIType;
      const context = (data.context as Record<string, unknown>) || {};

      if (!uiType) {
        return {
          success: false,
          error: 'Missing required parameter: uiType',
        };
      }

      const validTypes: DynamicUIType[] = ['welcome', 'notification', 'dialog', 'enhancement', 'warehouse', 'shop', 'custom'];
      if (!validTypes.includes(uiType)) {
        return {
          success: false,
          error: `Invalid uiType: ${uiType}. Valid types are: ${validTypes.join(', ')}`,
        };
      }

      const dynamicUI = await this.generateDynamicUI({
        type: uiType,
        context,
      });
